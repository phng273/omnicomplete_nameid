...
exports.evaluate = exports.renderPrompt = exports.resolveVariables = exports.generateVarCombinations = exports.DEFAULT_MAX_CONCURRENCY = void 0;
...
async function renderPrompt(prompt, vars, nunjucksFilters, provider) {
    const nunjucks = (0, util_1.getNunjucksEngine)(nunjucksFilters);
...
class Evaluator {
    ...
    async runEval({ provider, prompt, test, delay, nunjucksFilters: filters, evaluateOptions, }) {
        ...
        try {
            ...
            if (!response.cached) {
                ...
                if (sleep) {
                    logger_1.default.debug(`Sleeping for ${sleep}ms`);
            ...
            if (response.error) {
                ...
                const transform = test.options?.transform || test.options?.postprocess || provider.transform;
                if (transform) {
                    processedResponse.output = await (0, util_1.transformOutput)(transform, processedResponse.output, {
                        vars,
                        prompt,
                    });
                }
                (0, tiny_invariant_1.default)(processedResponse.output != null, 'Response output should not be null');
    ...
    async evaluate() {
        ...
        (0, azureopenaiUtil_1.maybeEmitAzureOpenAiWarning)(testSuite, tests);
        // Prepare vars
        ...
        for (let index = 0; index < tests.length; index++) {
            ...
            (0, tiny_invariant_1.default)(Array.isArray(testCase.assert || []), `testCase.assert is not an array in test case #${index + 1}`);
            ...
            const varCombinations = process.env.PROMPTFOO_DISABLE_VAR_EXPANSION || testCase.options.disableVarExpansion
                ? [testCase.vars]
        ...
        const createMultiBars = async (evalOptions) => {
            ...
            multibar = new cliProgress.MultiBar({
                format: '[{bar}] {percentage}% | ETA: {eta}s | {value}/{total} | {provider} "{prompt}" {vars}',
                hideCursor: true,
        ...
        if (this.options.interactiveProviders) {
            runEvalOptions = runEvalOptions.sort((a, b) => a.provider.id().localeCompare(b.provider.id()));
            logger_1.default.warn('Providers are running in serial with user input.');
            // Group evalOptions by provider
            const groupedEvalOptions = runEvalOptions.reduce((acc, evalOption) => {
            ...
            for (const [providerId, providerEvalOptions] of Object.entries(groupedEvalOptions)) {
                logger_1.default.info(`Running ${providerEvalOptions.length} evaluations for provider ${providerId} with concurrency=${concurrency}...`);
                if (this.options.showProgressBar) {
                    await createMultiBars(providerEvalOptions);
                }
                await async_1.default.forEachOfLimit(providerEvalOptions, concurrency, processEvalStep);
                if (multibar) {
                    multibar.stop();
                }
                // Prompt to continue to the next provider unless it's the last one
                if (Object.keys(groupedEvalOptions).indexOf(providerId) <
                    ...
                    await new Promise((resolve) => {
                        ...
                        rl.question(`\nReady to continue to the next provider? (Y/n) `, (answer) => {
                            ...
                            if (answer.toLowerCase() === 'n') {
                                logger_1.default.info('Aborting evaluation.');
                                process.exit(1);
                            }
                            resolve(true);
                        });
            ...
            await async_1.default.forEachOfLimit(runEvalOptions, concurrency, processEvalStep);
        }
        // Do we have to run comparisons between row outputs?
        const compareRowsCount = table.body.reduce((count, row) => count + (row.test.assert?.some((a) => a.type === 'select-best') ? 1 : 0), 0);
        let progressBar;
        ...
        for (let index = 0; index < table.body.length; index++) {
            ...
            if (compareAssertion) {
                ...
                const gradingResults = await (0, assertions_1.runCompareAssertion)(row.test, compareAssertion, outputs);
                ...
                if (progressBar) {
                    ...
                    logger_1.default.debug(`Model-graded comparison #${index + 1} of ${compareRowsCount} complete`);
                }
            }
        }
        // Finish up
