...
const BEDROCK_MODEL = {
    ...
    CLAUDE_MESSAGES: {
        params: (config, prompt, stop) => {
            const { system, extractedMessages } = (0, anthropic_1.parseMessages)(prompt);
            return {
    ...
    TITAN_TEXT: {
        params: (config, prompt, stop) => ({
            ...
            textGenerationConfig: {
                maxTokenCount: config?.textGenerationConfig?.maxTokenCount ||
                    parseInt(process.env.AWS_BEDROCK_MAX_TOKENS || '1024'),
                temperature: config?.textGenerationConfig?.temperature ||
