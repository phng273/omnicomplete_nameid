...
async function runAssertion({ prompt, provider, assertion, test, output, latencyMs, logProbs, cost, }) {
    ...
    if (assertion.transform) {
        output = await (0, util_1.transformOutput)(assertion.transform, output, {
            vars: test.vars,
    ...
    if (typeof renderedValue === 'string') {
        if (renderedValue.startsWith('file://')) {
            ...
            const filePath = path_1.default.resolve(basePath, renderedValue.slice('file://'.length));
            if (filePath.endsWith('.js') || filePath.endsWith('.cjs') || filePath.endsWith('.mjs')) {
                ...
                try {
                    const pythonScriptOutput = await (0, wrapper_1.runPython)(filePath, 'get_assert', [output, context]);
                ...
                renderedValue = JSON.parse(fs_1.default.readFileSync(path_1.default.resolve(basePath, filePath), 'utf8'));
            }
            else if (filePath.endsWith('.yaml') || filePath.endsWith('.yml')) {
                renderedValue = js_yaml_1.default.load(fs_1.default.readFileSync(path_1.default.resolve(basePath, filePath), 'utf8'));
            }
            else if (filePath.endsWith('.txt')) {
                // Trim to remove trailing newline
                renderedValue = fs_1.default.readFileSync(path_1.default.resolve(basePath, filePath), 'utf8').trim();
            }
            else {
