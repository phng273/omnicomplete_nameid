...
async function main() {
    ...
    program
        ...
        await (0, server_1.startServer)(cmdObj.port, cmdObj.apiBaseUrl, cmdObj.yes);
    });
    program
        .command('share')
        ...
        const createPublicUrl = async () => {
            const latestResults = await (0, util_1.readLatestResults)();
            if (!latestResults) {
    ...
    program
        ...
        (0, util_1.cleanupOldFileResults)(0);
        telemetry_1.default.record('command_used', {
            name: 'cache_clear',
        });
        await telemetry_1.default.send();
        ...
        const runEvaluation = async (initialization) => {
            ...
            if (outputPath) {
                ...
                if (typeof outputPath === 'string') {
                    await (0, util_1.writeOutput)(outputPath, summary, config, shareableUrl);
                }
                else if (Array.isArray(outputPath)) {
                    await (0, util_1.writeMultipleOutputs)(outputPath, summary, config, shareableUrl);
                }
            ...
            if (!cmdObj.write) {
                ...
                await (0, util_1.writeResultsToDatabase)(summary, config);
                if (shareableUrl) {
